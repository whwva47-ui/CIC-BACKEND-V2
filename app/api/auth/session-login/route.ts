...
export async function POST(req: NextRequest) {
  ...
  if (p.plan_status !== 'approved')
    return NextResponse.json(
      { reason: 'not_approved', error: 'Account not approved. Contact admin.' },
  ...
  if (p.plan === 'free' && (!p.trial_ends_at || new Date(p.trial_ends_at) < now))
    return NextResponse.json(
      { reason: 'expired', error: 'Your 7-day trial has ended. Upgrade to Basic ($8/mo) or Pro ($15/mo).', upgrade: true },
      { status: 403, headers: h }
    );

  if ((p.plan === 'basic' || p.plan === 'pro') && p.plan_expires_at && new Date(p.plan_expires_at) < now)
    return NextResponse.json(
      { reason: 'expired', error: 'Your plan has expired. Please renew.', upgrade: true },
      { status: 403, headers: h }
    );
  ...
  await db.from('active_sessions').upsert(
    {
      ...
      user_id: p.id,
      session_token: token,
      allow_multiple: op?.allow_multiple_devices ?? false,
  ...
  return NextResponse.json(
    {
      ...
      user: {
        ...
        max_daily_generations: p.max_daily_generations,
