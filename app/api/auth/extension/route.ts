import { NextRequest, NextResponse } from 'next/server';
import { createClient } from '@supabase/supabase-js';

export const dynamic = 'force-dynamic';

// Legacy auth route -- kept for v1.1.0 extension operators not yet updated
// New operators use /api/auth/session-login instead
const OPEN = { 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Methods': 'POST, OPTIONS', 'Access-Control-Allow-Headers': 'Content-Type, X-API-Key' };

export async function OPTIONS() { return new NextResponse(null, { status: 204, headers: OPEN }); }

export async function POST(req: NextRequest) {
  let email = '';
  try { const b = await req.json(); email = (b.email || '').trim().toLowerCase(); }
  catch { return NextResponse.json({ error: 'Invalid request.' }, { status: 400, headers: OPEN }); }
  if (!email.includes('@')) return NextResponse.json({ error: 'Valid email required.' }, { status: 400, headers: OPEN });

  const db = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.SUPABASE_SERVICE_ROLE_KEY!);
  const { data: p } = await db.from('profiles').select('id,email,plan,plan_status,daily_generations,max_daily_generations').eq('email', email).maybeSingle();

  if (!p) return NextResponse.json({ error: 'No account found. Sign up at chattersinnercircle.vercel.app' }, { status: 404, headers: OPEN });

  const remaining = p.plan === 'pro' ? 999999 : Math.max(0, (p.max_daily_generations || 10) - (p.daily_generations || 0));
  return NextResponse.json({ success: true, apiKey: 'legacy', user: { email: p.email, plan: p.plan, remaining } }, { headers: OPEN });
}
